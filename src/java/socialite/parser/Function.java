...
public class Function implements Param {
    ...
    void loadPyFunc() throws InternalException {
       ...
       // System.out.println(name);
        if (pyname.indexOf('.') >= 0) { System.out.println("which1");throw new InternalException("2Cannot find $" + pyname);}

        pyfunc = PyInterp.load(pyname);
        if (pyfunc == null) {System.out.println(pyname);throw new InternalException("1Cannot find $" + name);}
    ...
    void loadReally() throws InternalException {
        //fix this bug to support aggre function appear on the right of "=";
        //In old version socialite will move aggrfun to right side.
        // by qiange  unused
        String _method=null;
//        if(name.contains("dsum")){
//            System.out.println("xxxxxxxxx"+className()+" "+methodName());
//            klass = FunctionLoader.load("Dsum");
//            _method="apply";
//
//
//        }else{
//            _method=methodName();
  ...
  //      }
       ...
       // System.out.print(klass+"|."+methodName()+" coming"+" ");
        if (klass == null) {
            loadPyFunc();
            return;
        }
...
//        if(name.contains("dsum")&argTypesold.length<2){
//
//            argTypes=new Class<?>[argTypesold.length+1];
//            for(int i=0;i<argTypesold.length;i++){
//                argTypes[i]=argTypesold[i];
//            }
//            argTypes[argTypesold.length]=argTypesold[argTypesold.length-1];
//        }
//        else{
//            argTypes=argTypesold;
//        }
//        argTypes=argTypesold;
